/*
  # Coffee Marketing System Database Schema

  1. New Tables
    - `profiles` - User profile information linked to Supabase auth
    - `feedback` - Customer feedback records with coffee quality ratings
    - `sms_messages` - SMS message tracking with Twilio integration
    - `message_templates` - Reusable message templates for SMS
  ...
  3. Features
    - Coffee quality and delivery ratings (1-5 scale)
    - SMS status tracking (pending, sent, delivered, failed)
...
-- Create function to automatically create default message templates for new users
CREATE OR REPLACE FUNCTION create_default_templates_for_user()
RETURNS TRIGGER AS $$
BEGIN
  -- Insert default message templates for the new user
...
-- Create trigger to automatically create default templates when a new profile is created
